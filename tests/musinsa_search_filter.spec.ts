...
test('무신사 검색 -> 필터(총장) -> 상품선택 -> 옵션/장바구니 담기 (Final)', async ({ page, context }) => {
  ...
  // -----------------------------------------------------------------------
  // [수정됨] Step 1: 글로벌 리다이렉트 "강력 차단" & 검색
  // -----------------------------------------------------------------------
  await test.step('메인 페이지 진입 및 글로벌 차단', async () => {
    
    // 1. [핵심] "global.musinsa.com"으로 가는 모든 네트워크 요청을 차단(Abort)합니다.
    // 브라우저가 납치당해서 글로벌 사이트로 가려고 하면 "멈춰!" 하고 막습니다.
    await page.route('**/*global.musinsa.com*', async route => {
        console.log('🚫 글로벌 사이트 납치 시도 감지! 차단합니다.');
        await route.abort(); 
    });

    // 2. 한국 사이트 접속 시도
    // (차단 때문에 "페이지 로드 실패" 에러가 날 수도 있어서 try-catch로 감쌉니다)
    try {
        await page.goto(KR_URL, { timeout: 15000 });
    } catch (e) {
        console.log('⚠️ 페이지 이동 중 글로벌 차단됨 (하얀 화면이어도 한국 사이트 소스만 남았다면 성공)');
    }
    
    // 3. 타이틀 확인 (한국어 '무신사'가 있어야 성공)
    // 만약 여기서 실패하면, 무신사가 "미국 IP는 아예 한국 데이터 안 줌" 정책인 겁니다.
    await expect(page).toHaveTitle(/무신사/);
    console.log('✅ 한국 무신사 사이트 사수 성공!');

    // 4. 검색창 찾기 (한국 PC 버전 기준)
  ...
  // -----------------------------------------------------------------------
  // [이하 동일] Step 3 ~ Step 11은 기존 코드 그대로 유지
  // -----------------------------------------------------------------------
